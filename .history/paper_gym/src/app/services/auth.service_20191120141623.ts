  ...
  async login(user) {
    try {
      ...
      await loggedUser.user
        .getIdTokenResult()
        .then(response => {
          console.log({createdAt: Date.now(), expires: Date.parse(response.expirationTime)});
          token = response && response.token;
          localStorage.setItem("userToken", JSON.stringify({token: response.token, expiresAt: Date.parse(response.expirationTime)}))
        });
      return token;
    } catch (error) {
